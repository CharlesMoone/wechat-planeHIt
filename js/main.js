...
function game() {
  ...
  canvas.addEventListener('mousedown', (e) => {
    ...
    if (e.offsetX < plane.x - 25 || e.offsetX > plane.x + 25 || e.offsetY < plane.y - 25 || e.offsetY > plane.y + 25)
      return;
  ...
  canvas.addEventListener('mousemove', function (e) {
    ...
    if (isDead) {
      ...
      if (!document.getElementsByClassName('dead')[0]) {
        ...
        dead.className = 'dead';
        dead.innerHTML =
          '<table><tbody><tr><td><p>Game Over</p><p>your point : ' +
  ...
  var generateTime = Math.random() * 500,
    generatePosition = 0;
  ...
  setInterval(function () {
    ...
    enemyArray[enemyNumber] = new Enemy(
      enemyStartX * generatePosition + imgLoad[2].width,
      -imgLoad[2].height,
      imgLoad[2],
    );
...
function Enemy(x, y, img) {
  ...
  this.move = function (enemy, enemyNumber, ctx) {
    var enemyRun = setInterval(function () {
      ...
      if (
        (enemy.x - plane.x > 0 ? enemy.x - plane.x < 20 : plane.x - enemy.x < enemy.box.width + 20) &&
        (enemy.y - plane.y > 0 ? enemy.y - plane.y < 20 : plane.y - enemy.y < enemy.box.height + 20)
      ) {
        isDead = true;
      }
    }, 30);
